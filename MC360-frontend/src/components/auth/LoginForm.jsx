...
const LoginForm = ({ onSubmit, onGoogleLogin }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-6">Sign in to MedConnect360</p>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              value={form.password}
              onChange={handleChange}
              className="w-full border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 pr-10"
              placeholder="••••••••"
              required
            />
            <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-2.5 text-gray-400 text-xs">
              {showPass ? "Hide" : "Show"}
            </button>
          </div>
        </div>
      ...
      <button
        ...
        className="w-full border border-gray-300 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition flex items-center justify-center gap-2"
      >
